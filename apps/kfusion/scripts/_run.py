  ...
  """
    ...
    def run(self, dataset):
        ...
        self._run_internal(dataset.camera_file,
                           dataset.dataset_path, results_path, dataset.maskrcnn_folder)
    ...
    def _run_internal(self, camera_calib_path, dataset_path, results_path, maskrcnn_path):
        """ Generate the run command and run
        """
        stdio_log_path = results_path + '_log'
        with open(stdio_log_path, 'w') as stdio_log:
            if (maskrcnn_path == None):
                cmd = self._generate_run_command(
                    camera_calib_path, dataset_path, results_path)
            else:
                cmd = self._generate_run_of_command(
                    camera_calib_path, dataset_path, results_path, maskrcnn_path)
    ...
    def _plotTraj(self, filepath):
        self.evalATE.latest_plot(filepath + '.pdf')
        #xs = []
        #ys = []
        #gtxs = []
        #gtys = []
...
class KinectFusion(SLAMAlgorithm):
    ...
    def __init__(self, bin_path):
        ...
        self.voxel_block = '8'
        self.pyramid_levels = '10, 5, 4' #'20, 20, 20'
    ...
    def _generate_run_of_command(self, camera_calib_path, dataset_path, results_path, maskrcnn_path):
        args = []
        ...
        args.extend(['--mask-rcnn-folder', maskrcnn_path])
        if self.dump_volume != "":
